...
1. Create a basic web service project and open it in VS Code
    ...
    dotnet add package Microsoft.Extensions.Caching.Redis -v 2.2.0
    dotnet add package Steeltoe.Management.CloudFoundryCore -v 2.4.0
    dotnet add package Steeltoe.Extensions.Configuration.CloudFoundryCore -v 2.4.0
    dotnet add package Steeltoe.CloudFoundry.ConnectorCore -v 2.4.0
    dotnet add package Steeltoe.Extensions.Logging.DynamicLogger -v 2.4.0
    code .
    ```
...
1. Create a new Class `CrashService` in the `Services` directory, set its contents to the following:
...
1. Create a new class `ResetCountController` in the `Controllers` directory, set its contents to the following:
...
1. Create a new class `CrashController` in the `Controllers` directory, set its contents to the following:
...
1. Login to Pivotal Apps Manager at https://run.pivotal.io/ - or your local instance of Cloud Foundry
...
1. Create an Redis service instance in your PCF space. Name the service instance `xxxredis` where "xxx" is your initials
    ...
    - If you are using Pivotal Web Services, add a Redis Cloud instance using the 30MB (free) plan
